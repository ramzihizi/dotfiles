// "Rainbow smear" cursor trail for Ghostty — a hexagonal smear behind the
// cursor filled with animated, saturation-boosted rainbow plasma that fades
// out. Fires on cursor movement (including typing), so it's constantly visible.
//
// Vendored from https://github.com/KroneCorylus/ghostty-shader-playground
//   (public/shaders/cursor_smear_rainbow.glsl).
//
// Wired up in ../config via `custom-shader = shaders/cursor_smear_rainbow.glsl`
