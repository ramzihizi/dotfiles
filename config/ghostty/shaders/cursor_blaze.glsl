// "Blaze" cursor trail for Ghostty — a fiery yellow→red-orange smear that
// streaks behind the cursor on jumps.
//
// Vendored from https://github.com/0xhckr/ghostty-shaders (cursor_blaze.glsl),
// itself based on https://gist.github.com/chardskarth/95874c54e29da6b5a36ab7b50ae2d088.
//
// Wired up in ../config via `custom-shader = shaders/cursor_blaze.glsl` +
// `custom-shader-animation = always`. Uses a FIXED fire palette (TRAIL_COLOR /
// TRAIL_COLOR_ACCENT below) — it does NOT follow cursor-color, so recolor those
// two constants to change it (e.g. greens to match the hacker-green cursor).
// DURATION = trail lifetime; DRAW_THRESHOLD = min cursor jump (in cursor sizes)
// before the trail fires — lower it toward ~0.1 to also blaze on single-char
// typing.
