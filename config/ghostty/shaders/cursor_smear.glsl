// "Solid smear" cursor trail for Ghostty — a hexagonal smear behind the cursor
// filled with the actual cursor color (saturation-boosted) that fades out.
// Fires on cursor movement (including typing), so it's constantly visible. This
// is the plain, non-rainbow sibling of cursor_smear_rainbow.glsl: same hexagon
// geometry, but the trail follows cursor-color (green) instead of rainbow plasma
// — the classic kitty/Neovide look.
//
