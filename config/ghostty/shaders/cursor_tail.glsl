// Kitty-style "comet" trailing cursor for Ghostty.
//
// Vendored from https://github.com/sahaj-b/ghostty-cursor-shaders
//   (cursor_tail.glsl), MIT License, Copyright (c) 2026 Sahaj Bhatt.
//
// Wired up in ../config via `custom-shader = shaders/cursor_tail.glsl` +
// `custom-shader-animation = always`. The trail color follows the terminal's
// cursor-color automatically (iCurrentCursorColor). Tune the animation with
// the CONFIGURATION block below (DURATION, MAX_TRAIL_LENGTH, BLUR) and swap the
// active `ease()` by commenting/uncommenting one of the easing functions.
...
const float DURATION = 0.25; // in seconds (higher = slower trail; was 0.09)
const float MAX_TRAIL_LENGTH = 0.2;
const float THRESHOLD_MIN_DISTANCE = 0.1; // min distance to show trail (units of cursor width); low = smear even on single-char typing ("typewriter"); was 1.5
