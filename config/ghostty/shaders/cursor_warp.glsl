// "Warp" cursor trail for Ghostty — a smooth Neovide-style smear that stretches
// from the cursor's old position and eases into the new one, leaving a tapered
// trail. Follows cursor-color (green). Smooth and lingering, not sudden.
//
...
const float FADE_ENABLED = 1.0; // on = tapered gradient along the trail (more obvious); 0.0 = solid smear like Neovide's default
