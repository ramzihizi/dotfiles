// "Sonic boom" cursor effect for Ghostty — a shockwave ring that bursts from
// the cursor when its shape/size changes (e.g. vim normal<->insert). Fires on
// cursor-shape change, not on movement.
//
// Vendored from https://github.com/sahaj-b/ghostty-cursor-shaders
//   (sonic_boom_cursor.glsl), MIT License.
//
// Wired up in ../config via `custom-shader = shaders/sonic_boom_cursor.glsl` +
// `custom-shader-animation = always`. COLOR below is tinted to the terminal
// cursor color; DURATION and MAX_RADIUS tune the speed and size of the burst.
...
vec4 COLOR = vec4(iCurrentCursorColor.rgb, 1.0); // tinted to cursor-color (green); was vec4(0.35, 0.36, 0.44, 1.0)
